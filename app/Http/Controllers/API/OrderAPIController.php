...
class OrderAPIController extends Controller {
     ...
     */
    public function __construct(OrderRepository $orderRepo, FoodOrderRepository $foodOrderRepository, CartRepository $cartRepo, PaymentRepository $paymentRepo, NotificationRepository $notificationRepo, UserRepository $userRepository, FoodRepository $foodRepo, DriverRepository $driverRepo, ReferralRepository $referalRepo) {
        ...
        $this->referalRepository = $referalRepo;
    }
    ...
    private function cashPayment(Request $request) {
        ...
        try {
            ...
            $restaurant = $this->foodRepository->restaurantByFoodId($food_id);
            $drivers = $this->driverRepository->activeDriver();
            $uid = $this->_get_driver($restaurant['restaurant'], $drivers);
            // echo $uid;
            if ($uid != 0) {
                $driver = $this->userRepository->findWithoutFail($uid);
                if (!empty($driver)) {
    ...
    public function driver() {
        $order = $this->orderRepository->findWithoutFail(1);
        // print_r($order);
        $food_id = 1;
        $restaurant = $this->foodRepository->restaurantByFoodId($food_id);
        $drivers = $this->driverRepository->activeDriver();
    ...
    private function _get_driver($restaurant, $drivers) {
        $distance = 0;
        $id = 0;
        $lon = $restaurant['longitude']; //longitude of fixed point
        $lat = $restaurant['latitude'];
        ; //latitude of fixed point
        $radius = 6371; //radius of earth constant
        // formula for calculating Distance
        $phy_lat = ($lat * pi() / 180); //getting phy value of latitude

        foreach ($drivers as $key => $value) {
            $phy = ($value['latitude'] * pi() / 180);

            $del_lat = (($value['latitude'] - $lat) * pi() / 180); // Delta:meaning change
            $del_lon = (($value['longitude'] - $lon) * pi() / 180);

            $a = (sin($del_lat / 2) * sin($del_lat / 2)) + (cos($phy_lat) * cos($phy) * sin($del_lon / 2) * sin($del_lon / 2)); //formula for distance
            $a = 2 * atan2(sqrt($a), sqrt(1 - $a)); //$a is calculating distance

            $d = round($radius * $a * 1000); //multiplying 1000 for getting the distance in Meter, without *1000 distance will be shown in KM.

            if ($distance > $d || $distance == 0 && $distance < 7000) {
                $distance = $d;
                $id = $value['user_id'];
            }
        }
